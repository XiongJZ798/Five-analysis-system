...
//
//        // 测试空参数和有参数两种情况
//        Map<String, Object> resultEmpty = analysisService.getRateDistribution(null);
//        List<Map<String, Double>> ranges = new ArrayList<>();
//        Map<String, Double> range1 = Map.of("min", 0.0, "max", 100.0);
//        Map<String, Double> range2 = Map.of("min", 100.0, "max", 200.0);
//        ranges.add(range1);
//        ranges.add(range2);
//        Map<String, Object> resultWithRanges = analysisService.getRateDistribution(ranges);
//
//        long endTime = System.nanoTime();
//        long duration = TimeUnit.NANOSECONDS.toMillis(endTime - startTime);
//
...
//        assertTrue(resultEmpty.containsKey("pieData"));
//        assertNotNull(resultWithRanges);
//        assertTrue(resultWithRanges.containsKey("pieData"));
//
//        // 性能断言：处理时间应该小于1秒
...
//            data.setTestTime(baseTime.plusMinutes(i));
//            data.setRsrp(-85.0 - Math.random() * 10);
//            data.setSinr(15.0 + Math.random() * 5);
//            data.setMacThroughput(100.0 + Math.random() * 150); // 扩大速率范围
...
//            // 设置创建时间和更新时间
//            data.setCreateTime(baseTime.plusMinutes(i));
//            data.setUpdateTime(baseTime.plusMinutes(i));
//            dataList.add(data);
//        }
//
//        return dataList;
